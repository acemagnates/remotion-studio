...
- `durationInFrames?` â€” defaults to the parent sequence/composition duration. The effect reveals during the first half and retracts during the second half.
- `seed?` â€” determines the shape of the light leak pattern. Different seeds produce different patterns. Default: `0`.
- `hueShift?` â€” rotates the hue in degrees (`0`â€“`360`). Default: `0` (yellow-to-orange). `120` = green, `240` = blue.
...
- **Transitions** (`<TransitionSeries.Transition>`) â€” crossfade, slide, wipe, etc. between two scenes. Shortens the timeline because both scenes play simultaneously during the transition.
- **Overlays** (`<TransitionSeries.Overlay>`) â€” render an effect (e.g. a light leak) on top of the cut point without shortening the timeline.
...
- `presentation` â€” the visual effect (e.g. `fade()`, `slide()`, `wipe()`).
- `timing` â€” controls speed and easing (e.g. `linearTiming()`, `springTiming()`).

## Overlay props

`<TransitionSeries.Overlay>` accepts:

- `durationInFrames` â€” how long the overlay is visible (positive integer).
- `offset?` â€” shifts the overlay relative to the cut point center. Positive = later, negative = earlier. Default: `0`.
